...
class WOFHumanPlayer(WOFPlayer):
    def getMove(self,category, obscuredPhrase, guessed):
        ...
        print("Phrase: {}".format(self.obscuredPhrase))
        print("Guessed: {}".format(self.guessed))
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self,guessed):
        guess_let = []
        for i in LETTERS:
            if i in VOWELS and self.prizeMoney < VOWEL_COST:
                continue
            guess_let.append(i)
        return guess_let
    def getMove(self,category, obscuredPhrase, guessed):
        g = self.getPossibleLetters
        if g == [] or self.prizeMoney < VOWEL_COST:
            return 'pass'
        l = self.smartCoinFlip
        if l == True:
